...
def map_constraint_stages(scenario, scenario_tree, LP_symbol_map):
    ...
    for block in reference_model.block_data_objects(
        ...
        for constraint_data in block.component_data_objects(
                ...
                active=True,
                descend_into=False):
            raise TypeError("SOSConstraints are not allowed for this "
                            "output format. Invalid constraint: %s"
...
def _convert_explicit_setup(worker,
       ...
       (obj_repn.linear is not None):
        ...
        if StageToVariableMap[secondstage._name][0][0] not in \
           referenced_var_names:
            obj_vars.append(StageToVariableMap[secondstage._name][0][1])
            obj_coefs.append(0.0)
    ...
    with open(os.path.join(output_directory,
        ...
        if stochastic_rhs is not None:
            sorted_names = []
            for component in stochastic_rhs:
                if isinstance(component, _ConstraintData):
                    if component.active:
                        sorted_names.append(
                            (component.cname(True, name_buffer),
                             component,
                             stochastic_rhs[component]))
                elif isinstance(component, Constraint):
                    for index in component:
                        constraint_data = component[index]
                        if constraint_data.active:
                            sorted_names.append(
                                (constraint_data.cname(True, name_buffer),
                                 constraint_data,
                                 stochastic_rhs[component]))
                elif isinstance(component, _BlockData):
                    if component.active:
                        for constraint_data in component.component_data_objects(
                                Constraint,
                                active=True,
                                descend_into=True):
                            sorted_names.append(
                    ...
                    raise TypeError(
                        "Entries in PySP_StochasticRHS must be of "
                        "type Constraint or Block. Invalid type: %s"
                        % (type(component)))
                # IMPT: the check for 'is' must not be changed to '=='
                if not ((stochastic_rhs[component] is True) or
                        ((type(stochastic_rhs[component]) is tuple) and
                         (len(stochastic_rhs[component]) == 2) and
                         (stochastic_rhs[component] in ((True,False),
                                                        (True,True),
                                                        (False,True))))):
                    raise ValueError(
                        "Entries in PySP_StochasticRHS must be "
                        "assigned the value True or a 2-tuple with "
                        "at least one of the entries set to True. "
                        "Invalid value (%s) assigned to entry %s."
                        % (stochastic_rhs[component], component.cname(True)))
            sorted_names.sort(key=operator.itemgetter(0))
            for cname, constraint_data, include_bound in sorted_names:
        ...
        if stochastic_matrix is not None:
            sorted_names = []
            for component in stochastic_matrix:
                if isinstance(component, _ConstraintData):
                    if component.active:
                        sorted_names.append(
                            (component.cname(True, name_buffer),
                             component,
                             stochastic_matrix[component]))
                elif isinstance(component, Constraint):
                    for index in component:
                        constraint_data = component[index]
                        if constraint_data.active:
                    ...
                    raise TypeError(
                        "Entries in PySP_StochasticMatrix must be of "
                        "type Constraint or Block. Invalid type: %s"
                        % (type(component)))
            sorted_names.sort(key=operator.itemgetter(0))
            for cname, constraint_data, var_list in sorted_names:
                assert isinstance(constraint_data, _ConstraintData)
                # compute_values=False should prevent variables with a
                # zero coefficient from being eliminated from the
                # canonical representation
        ...
        if stochastic_objective is not None:
            ...
            if len(stochastic_objective) == 0:
                # assume every entry in the objective expression is
                # stochastic (including the constant)
                for var_data in objective_repn.variables:
                    sorted_names.append(
                        (var_data.cname(True, name_buffer), var_data))
                sorted_names.append(("ONE_VAR_CONSTANT", None))
            else:
                for component in stochastic_objective:
                    if isinstance(component, _VarData):
                        sorted_names.append(
                            (component.cname(True, name_buffer), component))
                    elif isinstance(component, Var):
                        for index in component:
                        ...
                        raise TypeError(
                            "Entries in PySP_StochasticObjective must be of "
                            "type Var or None. Invalid type: %s"
                            % (type(component)))
            sorted_names.sort(key=operator.itemgetter(0))
