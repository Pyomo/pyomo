...
from pyomo.environ import ConcreteModel, Param, Var, Objective, Expression, ConstraintList
...
class _SP_Feasible(object):
    ...
    def get_factory():
        ...
        model.x =  Var()
        model.Y =  Var([1])
        model.stale =  Var(initialize=0.0)
        model.fixed =  Var(initialize=0.0)
        model.fixed.fix()
        model.p =  Param(mutable=True)
        model.t0_cost =  Expression(expr=model.x)
        model.t1_cost =  Expression(expr=model.Y[1])
        model.o =  Objective(expr=model.t0_cost + model.t1_cost)
