...
class ProgressiveHedging(_PHBase):
    ...
    ### inner bound (upper bound if minimizing) calc ####
    def Inner_bound_and_make_a_mess(self):
        # compute the bound, but do not restore anything!
        import pyomo.pysp.plugins.phboundextension as phb
        Xhat = phb.ExtractInternalNodeSolutionsforInner(self)
        ...
        for node_name in Xhat:
            ...
            for variable_id, fix_val in iteritems(Xhat[node_name]):
                tree_node.fix_variable(variable_id, fix_val)
        print ("xxx hey!!! do I need to push something? xxxx")
        print ("see:     def FixScenarioTreeVariables(self, ph, fix_values) in phboundextension.py")
...
#                print "*** GAPS:",sorted(self._gaps.values())
    ...
    def solve(self):
        ...
        print("***********************************************************************************************")
        print ("Now attempting to give a valid number instead:")
        ObjBound, Xhat = self.Inner_bound_and_make_a_mess()
        if ObjBound is None:
            print ("No luck, so the expected sum of stage cost variables is probably bogus...")
        else:

            # the inner bound code left a "mess." I.e., the scenarios have their solutions
        ...
        print ("XXXX HEY: the inner bound code left a mess!!!! XXXXX")
