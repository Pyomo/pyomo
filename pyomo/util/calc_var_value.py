...
def calculate_variable_from_constraint(variable, constraint,
    ...
    if variable.value is None:
        if variable.lb is None:
            if variable.ub is None:
                # no variable values, and no lower or upper bound - set initial value to 0.0
                variable.set_value(0)
            else:
                # no variable value or lower bound - set to 0 or upper bound whichever is lower
                variable.set_value(min(0, variable.ub))
        elif variable.ub is None:
            # no variable value or upper bound - set to 0 or lower bound, whichever is higher
    ...
    while abs(value(expr)) > eps and iter_left:
        ...
        if abs(fpk) < 1e-12:
            raise RuntimeError("Newton's method encountered a derivative that was too close to zero."
        ...
        if linesearch:
            ...
            while alpha > 1e-8:
                # check if the value at xkp1 has sufficient reduction in the residual
                fkp1 = value(expr)
                if fkp1**2 < c1*fk**2:
            ...
            if alpha <= 1e-8:
                raise RuntimeError("Linesearch iteration limit reached. calc_var_value failed to converge.")
