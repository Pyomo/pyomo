...
from pyomo.core.expr.template_expr import GetItemExpression, GetAttrExpression, TemplateSumExpression, IndexTemplate
...
def handle_templateSumExpression_node(node,*args):
    pstr = ''
    pstr += '\\sum_{%s} %s'%('__SET_PLACEHOLDER_8675309_GROUP_%s_%s__'%( node._iters[0][0]._group, 
                                                                         str(node._iters[0][0]._set) ), args[0])
    return pstr
...
def latex_printer(pyomoElement, filename=None, useAlignEnvironment=False, splitContinuousSets=False):
    '''
    ...
    useAlignEnvironment: Default behavior uses equation/aligned and produces a single LaTeX Equation (ie, ==False).  
        ...
        objective and constraint.  Each objective and constraint will be labled with its name in the pyomo model.
        This flag is only relevant for Models and Blocks.
    
    splitContinuous: Default behavior has all sum indicies be over "i \in I" or similar.  Setting this flag to 
    ...
    if not isinstance(pyomoElement, _BlockData):
        ...
        objectives = [obj for obj in pyomoElement.component_data_objects(pe.Objective, descend_into=True, active=True)]
        constraints = [con for con in pyomoElement.component_objects(pe.Constraint, descend_into=True, active=True)]
        expressions = []
        templatize_fcn = templatize_constraint
    ...
    if len(constraints) > 0:
        ...
        for i in range(0,len(constraints)):
            ...
            # Walk the constraint
            conLine = ' '*tbSpc + algn + ' %s '%(visitor.walk_expression(con_template))
    ...
    # Handling the iterator indicies
    
    # preferential order for indices
    setPreferenceOrder = ['I','J','K','M','N','P','Q','R','S','T','U','V','W']
        ...
        if "PLACEHOLDER_8675309_GROUP_" in ln:
            ...
            for word in splitLatex:
                if "PLACEHOLDER_8675309_GROUP_" in word:
                    ...
                    gpNum, stNam = ifo.split('_')
                    if gpNum not in groupMap.keys():
                        groupMap[gpNum] = [stNam]
                    if stNam not in uniqueSets:
                        uniqueSets.append(stNam)

            # Determine if the set is continuous
            continuousSets = dict(zip(uniqueSets, [False for i in range(0,len(uniqueSets))] ))
            ...
            indexStrings = {}
            for ky, vl in groupMap.items():
                setStrings['__SET_PLACEHOLDER_8675309_GROUP_%s_%s__'%(ky,vl[0])] = [ vl[2], vl[1], vl[0] ]
                indexStrings['__INDEX_PLACEHOLDER_8675309_GROUP_%s_%s__'%(ky,vl[0])] = vl[2].lower()
            ...
            for ky, vl in setStrings.items():
                ...
                if splitContinuousSets and vl[1]:
                    ...
                    bgn = stData[0]
                    ed  = stData[-1]
                    ln = ln.replace('\\sum_{%s}'%(ky), '\\sum_{%s = %d}^{%d}'%(vl[0].lower(), bgn, ed))
                    ...
                    bgn = stData[0]
                    ed  = stData[-1]
                    ln = ln.replace('\\sum_{%s}'%(ky), '\\sum_{%s \\in %s}'%(vl[0].lower(),vl[0]))
                    ln = ln.replace(ky,vl[2])
