...
for f in glob.glob(datadir+'*_testCase.py'):
    ...
    if name in ['small16', 'small17']:
        BaselineTests.add_fn_test(fn=gams_writer_baseline_test, name=name, options={'determinism':0})
    else:
        BaselineTests.add_fn_test(fn=gams_writer_baseline_test, name=name, options={'determinism':1})
