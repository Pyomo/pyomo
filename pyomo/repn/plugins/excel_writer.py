...
class ProblemWriter_xlsx(AbstractProblemWriter):
    pyomo.util.plugin.alias('xlsx', 'Generate the corresponding Excel file')

    def __init__(self):
        AbstractProblemWriter.__init__(self, ProblemFormat.xlsx)
    ...
    def __call__(self,
        ...
        output_filename=None:
            Specify filename for Excel workbook. If None, will use model name.
            Must have extension .xlsx
        io_options:
            engine=None:
        ...
        engine = io_options.pop("engine", None)

        if len(io_options):
            raise ValueError(
                "ProblemWriter_xlsx passed unrecognized io_options:\n\t" +
                "\n\t".join("%s = %s"
                            % (k,v) for k,v in iteritems(io_options)))

        all_frames = self.write_dataframes(model)
        filename = self.write_excel(all_frames,
                                    filename=output_filename,
                                    engine=engine)
        return filename, None


    def write_dataframes(self, model):
        ...
        for obj in objects:
            if obj.type() in (Block, Disjunct) and obj.dim() == 0:
                obj_frames.append(self.write_dataframes(obj))
                continue
        ...
        for item in obj_frames:
            ...
            if type(item) is list:
                ...
                for blk in item[2:]:
                    ...
                    self._write_excel(blk, writer=writer, parent_row=parent_row,
