...
class TestSimpleDisjunctionTransformation(unittest.TestCase):
    def make_two_term_model(self):
        """A flat two-term Disjunction whose Disjuncts each have two Constraints.

        The two Constraints per Disjunct matter here: the transformation keeps
        just one of them.
        """
        m = ConcreteModel()
        m.x = Var(bounds=(0, 10))
    ...
    def make_nested_model(self):
        ...
        Used to exercise the (unsupported) nested cases.
        """
        m = ConcreteModel()
        m.x = Var(bounds=(0, 10))
        m.y = Var(bounds=(0, 10))
    ...
    def test_first_method_builds_simple_disjunction(self):
        ...
        self.assertIsInstance(simple, Disjunction)
    ...
    def test_multiple_disjunctions(self):
        ...
        TransformationFactory('gdp.simple_disjunction').apply_to(m)
        # Both Disjunctions share a single reformulation Block on their parent
        blocks = [
            b
        ...
        # ...and that one Block carries a simple Disjunction for each source
        simple_disjunctions = list(blocks[0].component_objects(Disjunction))
        self.assertEqual(len(simple_disjunctions), 2)
    ...
    def test_get_src_disjunction_bad_input_raises(self):
        ...
        with self.assertRaisesRegex(GDP_Error, "is a simple disjunction generated by"):
