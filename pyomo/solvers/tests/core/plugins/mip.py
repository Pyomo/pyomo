...
def mip_unique(results, data, options, name='unique-feasible'):
    ...
    if value != undefined:
        data['solution gap'] = isclose(value, 0.0, options.abstol, options.reltol), "'{0}' solution gap for {1} MIP solution".format(value, name)
    #
    ...
    if len(results.solution[0].objective) > 0:
        ...
        data['objective value'] = isclose(value, options._baseline.solution[0].objective[_objname].value, options.abstol, options.reltol), "'{0}' objective value for {2} MIP solution (baseline={1})".format(value, options._baseline.solution[0].objective[_objname].value, name)
...
def mip_feasible_solution(results, data, options, name):
    ...
    data['lower bound'] = isclose(value, options._baseline.problem[0].lower_bound, options.abstol, options.reltol), "'{0}' lower bound for {2} MIP (baseline={1})".format(value, options._baseline.problem[0].lower_bound, name)
    #
    value = results.problem[0].upper_bound
    data['upper bound'] = isclose(value, options._baseline.problem[0].upper_bound, options.abstol, options.reltol), "'{0}' upper bound for {2} MIP (baseline={1})".format(value, options._baseline.problem[0].upper_bound, name)
    #
