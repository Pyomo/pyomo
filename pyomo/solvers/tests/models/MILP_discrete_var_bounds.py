...
from pyomo.kernel import block, variable, objective, constraint, BooleanSet, IntegerSet
...
class MILP_discrete_var_bounds_kernel(MILP_discrete_var_bounds):
    ...
    def _generate_model(self):
        ...
        model.w2 =  variable(domain= BooleanSet)
        model.x2 =  variable(domain_type= IntegerSet,
                                lb=0, ub=1)
        model.yb =  variable(domain_type= IntegerSet,
                                lb=1, ub=1)
        model.zb =  variable(domain_type= IntegerSet,
                                lb=0, ub=0)
        model.yi =  variable(domain= IntegerSet, lb=-1)
        model.zi =  variable(domain= IntegerSet, ub=1)

        model.obj =  objective(model.w2 - model.x2 +\
                                  model.yb - model.zb +\
                                  model.yi - model.zi)

        model.c3 =  constraint(model.w2 >= 0)
        model.c4 =  constraint(model.x2 <= 1)
