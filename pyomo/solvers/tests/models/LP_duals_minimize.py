...
from pyomo.kernel import block, variable, objective, constraint, variable_dict, constraint_dict
...
class LP_duals_minimize_kernel(LP_duals_minimize):
    ...
    def _generate_model(self):
        ...
        model.x[2].lb = -1
        model.x[2].ub = 1
        model.obj =  objective(sum(model.x[i]*((-1)**(i+1))
                                      for i in model.s))
        ...
        model.c[9] =  constraint((-1.,model.x[9],-1.))
