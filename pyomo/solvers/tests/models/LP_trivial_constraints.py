...
class LP_trivial_constraints_kernel(LP_trivial_constraints):
    ...
    def _generate_model(self):
        ...
        model.x =  variable(domain=RealInterval(bounds=(float('-inf'), None)))
        model.y =  variable(ub=float('inf'))
        model.obj =  objective(model.x - model.y)
        model.c =  constraint_dict()
        model.c[1] =  constraint(model.x >= -2)
        model.c[2] =  constraint(model.y <= 3)
        cdata = model.c[3] =  constraint((0, 1, 3))
        assert cdata.lb == 0
        assert cdata.ub == 3
        assert cdata.body() == 1
        assert not cdata.equality
        cdata = model.c[4] =  constraint((0, 2, 3))
        assert cdata.lb == 0
        assert cdata.ub == 3
        assert cdata.body() == 2
        assert not cdata.equality
        cdata = model.c[5] =  constraint((0, 1, None))
        assert cdata.lb == 0
        assert cdata.ub is None
        assert cdata.body() == 1
        assert not cdata.equality
        cdata = model.c[6] =  constraint((None, 0, 1))
        assert cdata.lb is None
        assert cdata.ub == 1
        assert cdata.body() == 0
        assert not cdata.equality
        cdata = model.c[7] =  constraint((1,1))
        assert cdata.lb == 1
        assert cdata.ub == 1
        assert cdata.body() == 1
        assert cdata.equality
