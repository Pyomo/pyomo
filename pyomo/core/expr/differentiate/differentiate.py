...
def differentiate(expr, wrt=None, wrt_list=None, mode=diff_modes.reverse_numeric):
    ...
    This function returns the derivative of expr with respect to one or more variables.
    The type of the return value depends on the arguments wrt, wrt_list, and mode. See
    below for details.
    ...
    wrt: pyomo.core.base.var._GeneralVarData
        If specified, this function will return the derivative with respect to wrt. wrt is normally a
        _GeneralVarData, but could also be a _ParamData. wrt and wrt_list cannot both be specified.
    ...
    mode: pyomo.core.expr.differentiate.differentiate.diff_modes
        ...
        diff_modes enum:
            ...
            diff_modes.reverse_numeric:
                ...
                mode. If neither wrt nor wrt_list are specified, then a ComponentMap is returned where
                there will be a key for each node in the expression tree, and the values will be the
                floating point values of the derivatives at the current values of the variables.

    Returns
    -------
    res: float or pyomo.core.expr.numeric_expr.ExpressionBase or ComponentMap or list
    ...
    if mode == diff_modes.reverse_numeric or mode == diff_modes.reverse_symbolic:
        ...
        if wrt is not None:
            if wrt_list is not None:
                raise ValueError('differentiate(): Cannot specify both wrt and wrt_list.')
            if wrt in res:
                res = res[wrt]
            else:
                res = 0
