...
class TestZeroSumPropagate(unittest.TestCase):
    ...
    def test_non_negative_propagate(self):
        ...
        m2.v2 = Var(initialize=2, domain=NonNegativeReals)
        m2.v3 = Var(initialize=3, domain=NonNegativeReals)
        m2.v4 = Var(initialize=4, domain=NonNegativeReals)
        m2.c1 = Constraint(expr=m2.v1 >= m2.v2 + m2.v3 + m2.v4)
        m2.v1.fix()
        TransformationFactory('core.propagate_zero_sum').apply_to(m2)
        self.assertTrue(m2.v1.fixed)
        self.assertTrue(m2.v2.fixed)
        self.assertTrue(m2.v3.fixed)
        self.assertTrue(m2.v4.fixed)
        # m.display()
    ...
    def test_non_positive_propagate(self):
        ...
        m2 = ConcreteModel()
        m2.v1 = Var(initialize=0)
        m2.v2 = Var(initialize=-2, domain=NonPositiveReals)
        m2.v3 = Var(initialize=-3, domain=NonPositiveReals)
        m2.v4 = Var(initialize=-4, domain=NonPositiveReals)
        m2.c1 = Constraint(expr=m2.v1 <= m2.v2 + m2.v3 + m2.v4)
        m2.v1.fix()
        TransformationFactory('core.propagate_zero_sum').apply_to(m2)
        self.assertTrue(m2.v1.fixed)
