...
from pyomo.core.expr.current import (
    LinearExpression, SumExpression,
    MonomialTermExpression, ProductExpression, NPV_ProductExpression,
...
class TestExprGen(unittest.TestCase):
    ...
    def _run_cases(self, tests, op):
        try:
            for test_num, test in enumerate(tests):
                ...
                try:
                    mutable = [
                        isinstance(arg, _MutableSumExpression) for arg in args
                    ]
                    ...
                    for i, arg in enumerate(args):
                        self.assertFalse(
                            isinstance(arg, _MutableSumExpression)
                        )
                        if mutable[i]:
            ...
            logger.error(f"Failed test {test_num}: {test}")
            raise
    ...
    def test_product(self):
        tests = [
            ...
            (self.mutable_l0, self.mutable_l1,
             MonomialTermExpression((
                 NPV_ProductExpression((0, self.mon_npv.arg(0))),
                 self.mon_npv.arg(1),
             ))),
            (self.mutable_l0, self.mutable_l2,
             ProductExpression((0, self.mutable_l2))),
            (self.mutable_l0, self.asbinary,
             MonomialTermExpression((0, self.bin))),
            ...
            (self.mutable_l0, self.npv,
             NPV_ProductExpression((0, self.npv))),
            (self.mutable_l0, self.param, 0),
            (self.mutable_l0, self.param_mut,
             NPV_ProductExpression((0, self.param_mut))),
            (self.mutable_l0, self.var,
             MonomialTermExpression((0, self.var))),
            (self.mutable_l0, self.mon_native,
             MonomialTermExpression((0, self.mon_native.arg(1)))),
            (self.mutable_l0, self.mon_param,
             MonomialTermExpression((
                 NPV_ProductExpression((0, self.mon_param.arg(0))),
                 self.mon_param.arg(1),
             ))),
            (self.mutable_l0, self.mon_npv,
             MonomialTermExpression((
                 NPV_ProductExpression((0, self.mon_npv.arg(0))),
                 self.mon_npv.arg(1),
             ))),
            (self.mutable_l0, self.linear,
             ProductExpression((0, self.linear))),
            (self.mutable_l0, self.sum,
             ProductExpression((0, self.sum))),
            (self.mutable_l0, self.other,
             ProductExpression((0, self.other))),

            # 18
            (self.mutable_l1, self.mutable_l0,
             MonomialTermExpression((
                 NPV_ProductExpression((self.mon_npv.arg(0), 0)),
                 self.mon_npv.arg(1),
             ))),
            (self.mutable_l1, self.mutable_l1,
             ProductExpression((self.mon_npv, self.mon_npv))),
            (self.mutable_l1, self.mutable_l2,
             ProductExpression((self.mon_npv, self.mutable_l2))),
            (self.mutable_l1, self.asbinary,
             ProductExpression((self.mon_npv, self.bin))),
            (self.mutable_l1, self.invalid, NotImplemented),
            (self.mutable_l1, self.zero,
             MonomialTermExpression((
                 NPV_ProductExpression((self.mon_npv.arg(0), 0)),
                 self.mon_npv.arg(1),
             ))),
            (self.mutable_l1, self.one, self.mon_npv),
            (self.mutable_l1, self.native,
             MonomialTermExpression((
                 NPV_ProductExpression((self.mon_npv.arg(0), self.native)),
                 self.mon_npv.arg(1),
             ))),
            (self.mutable_l1, self.npv,
             MonomialTermExpression((
                 NPV_ProductExpression((self.mon_npv.arg(0), self.npv)),
                 self.mon_npv.arg(1),
             ))),
            # (self.mutable_l1, self.param, ),
            # (self.mutable_l1, self.param_mut, ),
            # (self.mutable_l1, self.var, ),
            # (self.mutable_l1, self.mon_native, ),
            # (self.mutable_l1, self.mon_param, ),
            # (self.mutable_l1, self.mon_npv, ),
            # (self.mutable_l1, self.linear, ),
            # (self.mutable_l1, self.sum, ),
            # (self.mutable_l1, self.other, ),
        ]
