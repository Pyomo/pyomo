...
class TestMiscPiecewise(unittest.TestCase):
    ...
    def test_activate_deactivate_indexed(self):
        ...
        self.assertTrue(len(model.c[1].components(Constraint)) > 0)
        self.assertTrue(len(model.c[1].active_components(Constraint)) > 0)
    ...
    def test_activate_deactivate_nonindexed(self):
        ...
        model.c = Piecewise(*args,**keywords)
        self.assertTrue(len(model.c.components(Constraint)) > 0)
        self.assertTrue(len(model.c.active_components(Constraint)) > 0)
        self.assertEqual(model.c.active, True)
        model.c.deactivate()
        self.assertTrue(len(model.c.components(Constraint)) > 0)
        self.assertTrue(len(model.c.active_components(Constraint)) > 0)
