...
from pyomo.environ import ConcreteModel, Var, exp, value
...
class TestTaylorSeries(unittest.TestCase):
    def test_first_order_taylor_series(self):
        ...
        m.x =  Var()
        m.x.value = 1
        exprs_to_test = [m.x**2,  exp(m.x), (m.x + 2)**2]
        for e in exprs_to_test:
            tsa = taylor_series_expansion(e)
            self.assertAlmostEqual( differentiate(e, wrt=m.x),  differentiate(tsa, wrt=m.x))
            self.assertAlmostEqual( value(e),  value(tsa))
    ...
    def test_higher_order_taylor_series(self):
        ...
        for _x in [-2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2]:
            for _y in [-2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2]:
                m.x.value = _x
                m.y.value = _y
                self.assertAlmostEqual( value(e),  value(tse))
