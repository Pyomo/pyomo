...
class RemoveZeroTerms(IsomorphicTransformation):
    ...
    def _apply_to(self, model):
        ...
        for constr in m.component_data_objects(
            ...
            nonzero_vars_indx = [
                i for i in range(len(repn.variables))
            ...
            if constr.equality:
                constr.set_value(
                    sum(repn.linear[i] * repn.variables[i]
                        for i in nonzero_vars_indx) + const == constr.upper)
            elif constr.has_lb() and not constr.has_ub():
                constr.set_value(
                    sum(repn.linear[i] * repn.variables[i]
                        for i in nonzero_vars_indx) + const >= constr.lower)
                ...
                constr.set_value(constr.lower <= sum(
                    repn.linear[i] * repn.variables[i]
                    for i in nonzero_vars_indx) + const <= constr.upper)
