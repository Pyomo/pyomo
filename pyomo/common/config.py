...
if 'PYOMO_CONFIG_DIR' in os.environ:
    PYOMO_CONFIG_DIR = os.environ['PYOMO_CONFIG_DIR']
elif platform.system().lower().startswith(('windows','cygwin')):
    PYOMO_CONFIG_DIR = os.path.join(os.environ.get('LOCALAPPDATA', ''), 'Pyomo')
else:
    PYOMO_CONFIG_DIR = os.path.join(os.environ.get('HOME', ''), '.pyomo')
