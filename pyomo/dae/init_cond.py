    ...
    for blk in model.component_objects(Block, active=True):
        ...
        for non_time_index in non_time_set:
            ...
            try:
                ...
                # Is there some equivalent Block.Skip-like object?
                if not suppress_warnings:
                    print(index_warning(blk.name, index))
                if not allow_skip:
                    raise
                continue
...
def solve_consistent_initial_conditions(model, time, solver):
    ...
    if not scheme == 'LAGRANGE-RADAU' or scheme == 'BACKWARD Difference':
        raise NotImplementedError(
            '%s discretization scheme is not supported' % scheme)

    t0 = time.first()
    timelist = [t for t in time if t != t0]
    was_originally_active = ComponentMap(
            [(comp, comp.active) for comp in 
                model.component_data_objects((Block, Constraint))])
