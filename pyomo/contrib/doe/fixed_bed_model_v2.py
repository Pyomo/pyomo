...
def create_model(temp=313.15, temp_bath=313.15, y=0.15, para=212, ua=5.0E6, opt = True, conti=False, optimize_trace=True, diff=0, eps=0.01, energy = True, doe_model=False, est_tr=False, est_ua=False, k_aug=False, temp_option=1, isotherm = True, chemsorb = True, physsorb = True, dispersion = False, fix_pres = False, v_fix=False):
    ''' 
    ...
    # declare perturbation set
    if diff is 0:
        pert_set = ['base']
    elif diff is 1:
        pert_set = ['base','forward_k','forward_ua']
    elif diff is -1:
        pert_set = ['base','backward_k','backward_ua']
    elif diff is 2:
        pert_set = ['forward_k','forward_ua','backward_k','backward_ua']
    else:
        print('Select for option "diff" is invalid!!!!')
    
    # assemble this set based on the options
    m.perturb = Set(initialize=pert_set)
    
    # define parameter value according to perturbation    
    def perturbations_k(m, i):
        if i is 'base' or 'forward_ua' or 'backward_ua':
            return 1.0*m.para
        elif i is 'forward_k':
            return (1.0 + m.eps)*m.para
    ...
    if not m.est_tr:
        if m.doe_model: 
            m.fitted_transport_coefficient = Var(m.perturb, bounds=(100, 400))
            m.fitted_transport_coefficient.fix(m.para)
        elif m.k_aug:
            m.fitted_transport_coefficient = Var(initialize=m.para)
...
def add_variables(m, dynamic=True, tf=3600, timesteps=None, start=0):
    '''
    Add variables to the Pyomo model using the toggles previously specified.
    
    Arguments: 
        m: the model 
        dynamic: determine if dynamic model or steady state model is used (for initialization approach 1)
        tf: the time scale [s]
    ...
    if dynamic:
        ...
        m.t = Set(initialize=[0])
          
    # Gas phase density (concentration) [mol/m^3]
    m.C = Var(m.perturb, m.COMPS, m.zgrid, m.t, bounds=den_bounds_pert)

    # Gas phase density derivative, [mol/m^3] / [s]
    m.dCdt = DerivativeVar(m.C, wrt=m.t)

    # Total gas phase density [mol/m3]
    m.total_den = Var(m.perturb, m.zgrid, m.t, bounds=(tden_low, tden_high))
        
    # Gas phase velocity [cm/s]
    m.v = Var(m.perturb, m.zgrid, m.t, initialize=0.1, bounds=bounds_velocity_pert)
    if m.v_fix:
        m.v.fix()
    ...
    # Gas pressure [bar]
    m.P = Var(m.perturb, m.zgrid, m.t,initialize=1, bounds=(P_low, P_high))
    if m.fix_pres: 
        m.P.fix()
    ...
    if m.energy:
        ...
        m.temp = Var(m.perturb, m.zgrid, m.t, initialize=m.temp_bath, bounds=(273.15,500.15), within=NonNegativeReals)
        ...
        def perturbations_ua(m, i):
            if i =='base' or i=='backward_k' or i=='forward_k':
                return 1.0*m.ua_init
            elif i is 'forward_ua':
                return (1.0 + m.eps)*m.ua_init
        ...
        if not m.est_ua:
            ...
            if m.doe_model:
                m.ua =  Var(m.perturb, bounds=(1, 1.0E8))
                m.ua.fix(m.ua_init)
            elif m.k_aug:
                m.ua = Var(initialize=m.ua_init)
                #m.ua.setlb(m.ua_init)
                #m.ua.setub(m.ua_init)
            else:
                m.ua = Expression(m.perturb, rule=perturbations_ua)
        else:
        ...
        if (not m.est_tr) and (not m.k_aug):
            m.inv_K_oc = Expression(m.perturb, m.zgrid, m.t, rule=inv_k_oc_init_en)
            m.inv_K_op = Expression(m.perturb, m.zgrid, m.t, rule=inv_k_op_init_en)
        ...
        m.cpg = Expression(m.perturb, m.COMPS, m.zgrid, m.t, rule=cpg_rule)
        ...
        m.h = Expression(m.perturb, m.COMPS, m.zgrid, m.t, rule=h_rule)
    ...
    if m.isotherm:
        ...
        m.spp = Var(m.perturb, m.SCOMPS, m.zgrid, m.t, initialize=small_initial, bounds=surface_partial_pressure_bounds_pert)
        
        # Chemical adsorption equilibrium, [mol / kg]
        m.nchemstar = Var(m.perturb, m.SCOMPS, m.zgrid, m.t, initialize = small_initial, bounds=chem_star_bounds_pert)
        
        # Chemical adsorption equilibrium, modified [mol / kg]
        m.nchemstar_mod = Var(m.perturb, m.SCOMPS, m.zgrid, m.t, initialize = small_initial, bounds=chem_star_bounds_pert)
        
        # Physical adsorption equilibrium, [mol / kg]
        m.nphysstar = Var(m.perturb, m.SCOMPS, m.zgrid, m.t, initialize = small_initial, bounds=phys_star_bounds_pert)
        
        # Physical adsorption equilibrium with linear region [mol/kg]
        m.nphysstar_mod = Var(m.perturb, m.SCOMPS, m.zgrid, m.t, initialize =small_initial, bounds=phys_star_bounds_pert)
        
        # alpha used to form linearized pressure
        if alpha_variable:
            m.alpha = Var(m.perturb, m.SCOMPS, m.zgrid, m.t, initialize = small_initial, bounds=(small_bound,1.0))

    if m.chemsorb:
         
        # Chemical adsorption loading, [mol of gas/ kg of sorbent]
        m.nchem = Var(m.perturb, m.SCOMPS, m.zgrid, m.t, initialize = small_initial, bounds=chem_star_bounds_pert)

        # Time derivative, [mol/kg/s]
        if dynamic:
            m.dnchemdt = DerivativeVar(m.nchem, wrt=m.t)
        else:
            m.dnchemdt = Var(m.perturb, m.SCOMPS, m.zgrid, m.t, initialize = small_initial)
        
    if m.physsorb:
        # Physical adsorption equilibrium loading, [mol of gas/kg of sorbent]
        m.nphys = Var(m.perturb, m.SCOMPS, m.zgrid, m.t, initialize = small_initial, bounds=phys_star_bounds_pert)
        
        # Time derivative, [mol/kg/s]
        if dynamic:
            m.dnphysdt = DerivativeVar(m.nphys, wrt=m.t)
        else:
            m.dnphysdt = Var(m.perturb, m.SCOMPS, m.zgrid, m.t, initialize = small_initial)
    ...
    if m.energy:
        ...
        # LHS: [1/bar]
        # RHS: [1/bar] * exp( [kJ/mol]/[kJ/mol/K * K] ) = [1/b] * exp( [ dimensionless ] )
        m.b_a = Expression(m.perturb, m.zgrid, m.t, rule=b_a_en) 
        ...
        m.n_a = Expression(m.perturb, m.zgrid, m.t, rule=n_a_en)

        def inv_n_a_en(m,j,z,t):
            return 1/m.n_a[j,z,t]
        m.inv_n_a = Expression(m.perturb, m.zgrid, m.t, rule=inv_n_a_en)
        ...
        m.K_eq = Expression(m.perturb, m.zgrid, m.t, rule=k_eq_en)
        ...
        m.b_b = Expression(m.perturb, m.zgrid, m.t, rule=b_b_en)
        ...
        m.nmax_p = Expression(m.perturb, m.zgrid, m.t, rule=nmax_p_en)
        ...
        m.nmax_c = Expression(m.perturb, m.zgrid, m.t, rule=nmax_c_en)
        ...
        m.nplin = Expression(m.perturb, m.zgrid, m.t, rule=nplin_en)
...
def add_equations(mod):
    ...
    mod.dalton_law = Constraint(mod.perturb, mod.zgrid, mod.t, rule=dalton)

    mod.ideal_gas_law = Constraint(mod.perturb, mod.zgrid, mod.t, rule=ideal)
    
    # measurements
    if not mod.k_aug:
        mod.FCO2 = Expression(mod.perturb, mod.zgrid, mod.t, rule=FCO2_calc)
