...
def run_reactor_update_suffix_items():
    ...
    if example_suffix not in reactor_model.component_map():
        raise ValueError(
            f"The suffix '{example_suffix}' is not present in the reactor model."
        )
    ...
    utils.update_model_from_suffix(reactor_model, example_suffix, [1, 0.5, 0.1, 1])
    ...
    print({comp.name: tag for comp, tag in reactor_model.unknown_parameters.items()})
