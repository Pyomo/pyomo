...
class TestOptimizeExperimentsBuildStructure(unittest.TestCase):
    ...
    def test_optimize_experiments_greybox_uses_aggregated_fim(self):
        # Tests that the multi-experiment grey box path is built on the
        ...
        total_fim_np = np.array(doe_obj.results["Scenarios"][0]["Total FIM"])
        parameter_names = list(scenario.exp_blocks[0].parameter_names)

        # The grey box stores only one triangular copy of the symmetric FIM.
        # Check that those inputs were initialized from the aggregated FIM that
        # optimize_experiments() uses for the scenario objective.
        ...
        expected_min_eig = np.min(np.linalg.eigvalsh(total_fim_np))
        self.assertAlmostEqual(
