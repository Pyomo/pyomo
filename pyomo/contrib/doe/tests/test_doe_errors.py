...
class TestReactorExampleErrors(unittest.TestCase):
    ...
    def test_reactor_check_no_get_labeled_model(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "The experiment object must have a ``get_labeled_model`` function"
        ):
    ...
    def test_reactor_check_bad_prior_size(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Shape of FIM provided should be n parameters by n parameters, or {} by {}, FIM provided has shape {} by {}".format(
    ...
    def test_reactor_check_bad_jacobian_init_size(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Shape of Jacobian provided should be n experiment outputs by n parameters, or {} by {}, Jacobian provided has shape {} by {}".format(
    ...
    def test_reactor_check_unbuilt_update_FIM(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "``fim`` is not defined on the model provided. Please build the model first."
        ):
    ...
    def test_reactor_check_measurement_and_output_length_match(self):
        fd_method = "central"
        obj_used = "trace"
        flag_val = 5  # Value for faulty model build mode - 5: Mismatch error and output length
        ...
        with self.assertRaisesRegex(
            ValueError, "Number of experiment outputs, {}, and length of measurement error, {}, do not match. Please check model labeling.".format(
            27, 1
        )):
            doe_obj.create_doe_model()
    ...
    def test_reactor_grid_search_des_range_inputs(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Design ranges keys must be a subset of experimental design names."
        ):
    ...
    def test_reactor_premature_figure_drawing(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "Results must be provided or the compute_FIM_full_factorial function must be run."
        ):
    ...
    def test_reactor_figure_drawing_no_des_var_names(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "If results object is provided, you must include all the design variable names."
        ):
    ...
    def test_reactor_figure_drawing_bad_fixed_names(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Fixed design variables do not all appear in the results object keys."
        ):
            doe_obj.draw_factorial_figure(sensitivity_design_variables={"CA[0]": 1}, fixed_design_variables={"bad": "entry"})
    ...
    def test_reactor_figure_drawing_bad_sens_names(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Sensitivity design variables do not all appear in the results object keys."
        ):
            doe_obj.draw_factorial_figure(sensitivity_design_variables={"bad": "entry"}, fixed_design_variables={"CA[0]": 1})
    ...
    def test_reactor_check_get_FIM_without_FIM(self):
        fd_method = "central"
        obj_used = "trace"
        flag_val = 0  # Value for faulty model build mode - 5: Mismatch error and output length
        ...
        with self.assertRaisesRegex(
            RuntimeError, "Model provided does not have variable `fim`. Please make sure the model is built properly before calling `get_FIM`"
        ):
            doe_obj.get_FIM()
    ...
    def test_reactor_check_get_sens_mat_without_model(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "Model provided does not have variable `sensitivity_jacobian`. Please make sure the model is built properly before calling `get_sensitivity_matrix`"
        ):
    ...
    def test_reactor_check_get_exp_inputs_without_model(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "Model provided does not have expected structure. Please make sure model is built properly before calling `get_experiment_input_values`"
        ):
