...
class Scenario_generator:
    def __init__(self, para_dict, formula='central', step=0.001, store=False):
        ...
        if formula not in ['central', 'forward', 'backward', None]:
            raise ValueError(
                'Undefined formula. Available formulas: central, forward, backward, none.'
            )
    ...
    def simultaneous_scenario(self):
        ...
        scenario_object = Scenario_data(
            self.para_dict, self.scena_keys, self.scena, self.formula, self.step
        )
    ...
    def next_sequential_scenario(self, count):
        ...
        scenario_object = Scenario_data(
            self.scena[count], scena_keys, scena, None, self.step
        )
