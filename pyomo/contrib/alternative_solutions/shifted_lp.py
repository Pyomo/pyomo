...
def get_shifted_linear_model(model, block=None):
    ...
    var_range = {}
    for var in all_vars:
        assert var.lb is not None , ('Variable {} does not have a '
                                     'lower bound. All variables must be '
                                     'bounded.'.format(var.name))
        assert var.ub is not None , ('Variable {} does not have an '
    ...
    shifted_lp.var_lower = pe.Var(new_vars.keys(), domain=pe.NonNegativeReals, 
                                bounds=var_range)
    shifted_lp.var_upper = pe.Var(new_vars.keys(), domain=pe.NonNegativeReals, 
                                bounds=var_range)
    ...
    for constraint in model.component_data_objects(pe.Constraint, active=True):
        ...
        if constraint.equality:
            ...
            constraint_name = _get_unique_name(new_constraints.keys(), 
                                               constraint.name)
    ...
    shifted_lp.slack_vars = pe.Var(shifted_lp.slack_index, 
