...
class TestBalasUnit:
    ...
    def test_no_time(self, mip_solver):
        ...
        with pytest.raises(Exception):
            results = enumerate_binary_solutions(m, num_solutions=100, solver=mip_solver, solver_options={"TimeLimit":0})
    ...
    def test_knapsack_all(self, mip_solver):
        ...
        """
        m = tc.get_aos_test_knapsack(1, weights=[3,4,6,5], values=[2,3,1,4], capacity=8)
        results = enumerate_binary_solutions(m, num_solutions=100, solver=mip_solver)
        objectives = list(sorted((round(result.objective[1], 2) for result in results), reverse=True))
    ...
    def test_knapsack_x0_x1(self, mip_solver):
        ...
        """
        m = tc.get_aos_test_knapsack(1, weights=[3,4,6,5], values=[2,3,1,4], capacity=8)
        results = enumerate_binary_solutions(m, num_solutions=100, solver=mip_solver, variables=[m.x[0], m.x[1]])
        objectives = list(sorted((round(result.objective[1], 2) for result in results), reverse=True))
    ...
    def test_knapsack_hamming_3(self, mip_solver):
        ...
        """
        m = tc.get_aos_test_knapsack(1, weights=[3,4,6,5], values=[2,3,1,4], capacity=8)
        results = enumerate_binary_solutions(m, num_solutions=3, solver=mip_solver, search_mode="hamming")
        objectives = list(sorted((round(result.objective[1], 2) for result in results), reverse=True))
    ...
    def test_knapsack_random_3(self, mip_solver):
        ...
        """
        m = tc.get_aos_test_knapsack(1, weights=[3,4,6,5], values=[2,3,1,4], capacity=8)
        results = enumerate_binary_solutions(m, num_solutions=3, solver=mip_solver, search_mode="random", seed=1118798374)
        objectives = list(sorted((round(result.objective[1], 2) for result in results), reverse=True))
        assert_array_almost_equal(objectives, [6, 4, 1])
