...
from pyomo.contrib.alternative_solutions import PoolManager, Solution, Variable, Objective


def soln(value, objective):
    return Solution(variables=[Variable(value=value)], objectives=[Objective(value=objective)])


def test_keepall_add():
    pm = PoolManager()
    pm.reset_solution_counter()
    ...
    assert pm.to_dict() == \
    {'pool': {'metadata': {'context_name': 'pool'},
            'pool_config': {'policy': 'keep_all'},
            'solutions': {0: {'id': 0,
                              'objectives': [{'name': None,
                                              'suffix': {},
                                              'value': 0}],
                              'suffix': {},
...
def test_keepbest_add1():
    ...
    assert pm.to_dict() == \
    {'pool': {'metadata': {'context_name': 'pool'},
            'pool_config': {'abs_tolerance':1, 'max_pool_size':None, 'objective':None, 'policy': 'keep_best', 'rel_tolerance':None},
            'solutions': {0: {'id': 0,
                              'objectives': [{'name': None,
                                              'suffix': {},
                                              'value': 0}],
