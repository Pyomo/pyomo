...
from pyomo.environ import ConcreteModel, Var, Objective, Constraint, Set, ComponentMap, SolverFactory
import time
...
def create_master(farmer):
    ...
    m.crops =  Set(initialize=farmer.crops, ordered=True)
    m.scenarios =  Set(initialize=farmer.scenarios, ordered=True)

    m.devoted_acreage =  Var(m.crops, bounds=(0, farmer.total_acreage))
    m.eta =  Var(m.scenarios)
    for s in m.scenarios:
        m.eta[s].setlb(-432000 * farmer.scenario_probabilities[s])

    m.total_acreage_con =  Constraint(expr=sum(m.devoted_acreage.values()) <= farmer.total_acreage)

    m.obj =  Objective(expr=sum(farmer.PlantingCostPerAcre[crop] * m.devoted_acreage[crop] for crop in m.crops) + sum(m.eta.values()))
    return m
...
def create_subproblem(master, farmer, scenario):
    ...
    m.devoted_acreage =  Var(m.crops)
    m.QuantitySubQuotaSold =  Var(m.crops, bounds=(0.0, None))
    m.QuantitySuperQuotaSold =  Var(m.crops, bounds=(0.0, None))
    m.QuantityPurchased =  Var(m.crops, bounds=(0.0, None))
    ...
    m.EnforceCattleFeedRequirement =  Constraint(m.crops, rule=EnforceCattleFeedRequirement_rule)
    ...
    m.LimitAmountSold =  Constraint(m.crops, rule=LimitAmountSold_rule)

    def EnforceQuotas_rule(m, i):
        return (0.0, m.QuantitySubQuotaSold[i], farmer.PriceQuota[i])
    m.EnforceQuotas =  Constraint(m.crops, rule=EnforceQuotas_rule)
    ...
    m.obj =  Objective(expr=farmer.scenario_probabilities[scenario] * obj_expr)
