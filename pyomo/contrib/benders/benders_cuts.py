...
from pyomo.environ import ComponentMap, Var, Suffix, Objective, Constraint, ConstraintList, SolverFactory, TerminationCondition, value
...
def _setup_subproblem(b, master_vars, relax_subproblem_cons):
    ...
    objs = list(b.component_data_objects( Objective, descend_into=False, active=True))
    if len(objs) != 1:
    ...
    b._z =  Var(bounds=(0, None))
    b.objective =  Objective(expr=b._z)
    b.dual =  Suffix(direction= Suffix.IMPORT)
    b._eta =  Var()

    b.aux_cons =  ConstraintList()
    for c in list(b.component_data_objects( Constraint, descend_into=True, active=True, sort=True)):
    ...
    b.obj_con =  Constraint(expr=orig_obj_expr - b._eta - b._z <= 0)
...
class BendersCutGeneratorData(_BlockData):
    def __init__(self, component):
        ...
        self.master_vars_indices =  ComponentMap()
    ...
    def add_subproblem(self, subproblem_fn, subproblem_fn_kwargs, master_eta, subproblem_solver='gurobi_persistent', relax_subproblem_cons=False):
        ...
        if _rank == self.comm.Get_rank():
            ...
            if isinstance(subproblem_solver, str):
                subproblem_solver =  SolverFactory(subproblem_solver)
    ...
    def generate_cut(self):
        ...
        for local_subproblem_ndx in range(len(self.subproblems)):
            ...
            subproblem.fix_complicating_vars =  ConstraintList()
            ...
            subproblem.fix_eta =  Constraint(expr=subproblem._eta - master_eta.value == 0)
            ...
            constants[global_subproblem_ndx] =  value(subproblem._z)
            eta_coeffs[global_subproblem_ndx] = sign_convention *  value(subproblem.dual[subproblem.obj_con])
            for master_var in self.master_vars:
                if master_var in complicating_vars_map:
                    c = var_to_con_map[master_var]
                    coefficients[coeff_ndx] = sign_convention *  value(subproblem.dual[c])
                coeff_ndx += 1
