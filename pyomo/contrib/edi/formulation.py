...
class Formulation(ConcreteModel):
            ...
            if isinstance(size,(list, tuple)):
                for i in range(0,len(size)):
                    ...
                    if i == 0:
                        ...
                        st *= pyo.Set(initialize=list(range(0,size[i])))
                st.construct()
                self.add_component(name, pyo.Var(st, name=name, initialize=guess, domain=domain, bounds=bounds, doc=description, units=decodeUnits(units)))
            else:
                if isinstance(size, int):
                    if size == 1 or size == 0:
                        self.add_component(name, pyo.Var(name=name, initialize=guess, domain=domain, bounds=bounds, doc=description, units=decodeUnits(units)))
                    else:
            ...
            if isinstance(size,(list, tuple)):
                for i in range(0,len(size)):
                    ...
                    if i == 0:
                        ...
                        st *= pyo.Set(initialize=list(range(0,size[i])))
                st.construct()
                self.add_component(name, pyo.Param(st, name=name, initialize=value, within=within, doc=description, units=decodeUnits(units), mutable=True))
            else:
                if isinstance(size, int):
                    if size == 1 or size == 0:
                        self.add_component(name, pyo.Param(name=name, initialize=value, within=within, doc=description, units=decodeUnits(units), mutable=True))
                    else:
                        st = pyo.Set(initialize=list(range(0,size)))
                        st.construct()
                        self.add_component(name, pyo.Param(st, name=name, initialize=value, within=within, doc=description, units=decodeUnits(units), mutable=True))
    ...
    def Objective(self, expr, sense=minimize):
        ...
        self.add_component('objective_'+str(self._objective_counter) , pyo.Objective(expr=expr,sense=sense))
