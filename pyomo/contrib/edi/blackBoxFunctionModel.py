            ...
            for i in range(0,len(valueList)):
                ...
                if isinstance(vl, pyomo.core.expr.numeric_expr.NumericNDArray):
                    ...
                    for j in range(0,len(validIndexList)):
                        vi = validIndexList[j]
                        corrected_value = pyo.value(pyomo_units.convert(vl[vi], outputOptimizationUnits)) # now unitless in correct units
                    ...
                    corrected_value = pyo.value(pyomo_units.convert(vl, outputOptimizationUnits)) # now unitless in correct units
                    outputVector.append(corrected_value) 
        ...
        # ------------------------------
            ...
            if any([ list(kwargs.keys())[i] in inputNames for i in range(0,len(list(kwargs.keys()))) ]):
                ...
                if len(args) >= len(inputNames):
                    raise ValueError('A keyword input is defining an input, but there are too many unkeyed arguments for this to occur.  Check the inputs.')
                else:
                    if len(args) != 0:
