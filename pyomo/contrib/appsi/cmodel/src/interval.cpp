...
void _inverse_power2(double zl, double zu, double xl, double xu, double* yl, double* yu, double feasibility_tol)
{
  /*
    ...
    *res_lb = yl;
  else if (_sin_with_inf(yl) < xl)
    {
      ...
      */
      ...
      double y_tmp = std::asin(xl);  // this will give me a value between -pi/2 and pi/2
      double dist = y_tmp - (-pi / 2);  // this is the distance between the minimum of the sin function and a value that
      // satisfies xl = sin(y)
      double lb1 = i1 + dist;
      double lb2 = i2 + dist;
    ...
    *res_lb = yl;
  else if (_cos_with_inf(yl) < xl)
    {
      ...
      */
      ...
      double dist = pi - y_tmp;  // this is the distance between the minimum of the sin function and a value that
      // satisfies xl = sin(y)
      double lb1 = i1 + dist;
      double lb2 = i2 + dist;
      if (lb1 >= yl - feasibility_tol)
...
{
  double res_lb, res_ub;
  _inverse_power1(zl, zu, yl, yu, orig_xl, orig_xu, &res_lb, &res_ub, feasibility_tol);
  return std::make_pair(res_lb, res_ub);
}
