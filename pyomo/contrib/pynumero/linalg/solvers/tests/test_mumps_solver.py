...
class TestMUMPSSymLinearSolver(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        data = np.array([1, 7, 3, 4, -5, 6], dtype='d')
        ...
        cls.basic_rhs = np.array([1, 1, 0], dtype='d')

        # block algebra
        row = np.array([0, 1])
        col = np.array([0, 1])
        data = np.array([2, 8], dtype='d')
        H = coo_matrix((data, (row, col)), shape=(2, 2))

        row = np.array([0, 1, 0])
        col = np.array([0, 0, 1])
        data = np.array([1, 1, 1], dtype='d')
        A = coo_matrix((data, (row, col)), shape=(2, 2))

        cls.block_m = BlockSymMatrix(2)
        cls.block_m[0, 0] = H
        cls.block_m[1, 0] = A
        cls.block_rhs = BlockVector([np.array([8, 16], dtype='d'),
                                     np.array([-5, -3], dtype='d')])
    ...
    def test_num_negative_eigvals(self):

        linear_solver2 = MUMPSSymLinearSolver()
        A = self.block_m.tocoo()
        linear_solver2.do_symbolic_factorization(A)
        linear_solver2.do_numeric_factorization(A)
        self.assertEqual(linear_solver2._get_num_neg_evals(), 2)
