...
def newton_unconstrained(nlp, **kwargs):
    ...
    assert np.all(np.isinf(nlp.xl)), 'Newton unconstrained does not support lower bounds'
    assert np.all(np.isinf(nlp.xu)), 'Newton unconstrained does not support upper bounds'
...
def newton_unconstrained2(nlp, **kwargs):
