...
def basic_sqp(nlp, **kwargs):
    ...
    if nlp.ng == 0 and np.all(np.isinf(nlp.xl)) and np.all(np.isinf(nlp.xu)):
        sqp_logger.info("Solve as unconstrained problem")
        return newton_unconstrained(nlp, tee=tee, **kwargs)

    if not np.all(np.isinf(nlp.xl)) and np.all(np.isinf(nlp.xu)):
...
def basic_sqp2(nlp, **kwargs):
    ...
    if nlp.ng == 0 and nxlb == 0 and nxub == 0:
        sqp_logger.info("Solve as unconstrained problem")
        return newton_unconstrained2(nlp, tee=tee, **kwargs)
