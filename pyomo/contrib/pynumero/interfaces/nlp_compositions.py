from pyomo.contrib.pynumero.interfaces.nlp import NLP, NLPgcd
...
class TwoStageStochasticNLP(NLP):
    ...
    def __init__(self,
        ...
        complicated_vars: dictionary with complicated variables
        (scenario names to list of variable indices)

        """
        if not isinstance(nlps, dict):
            raise RuntimeError("Model must be a dictionary")
        if not isinstance(complicated_vars, dict):
            raise RuntimeError("complicated_vars must be a dictionary")
        if len(complicated_vars) != len(nlps):
            raise RuntimeError("Each scenario must have a list of complicated variables")

        aux_counter = 0
        n_z = 0
        for k, l in complicated_vars.items():
            if k not in nlps:
        ...
        for sid, sname in enumerate(self._sid_to_sname):
            self._zid_to_vid.append(complicated_vars[sname])
...
class TwoStageStochasticNLP2(NLPgcd):
    """
    Nonlinear program interface for composite NLP that result from
    two-stage stochastic programming problems
    """
    ...
    def __init__(self,
        ...
        # call parent class to set model
        super(TwoStageStochasticNLP2, self).__init__(None)
