...
class TestGDPBounds(unittest.TestCase):
    ...
    def test_enforce_bounds(self):
        ...
        m.d1.d = Disjunct()
        m.d2.d = Disjunct()
        m.d1._disj_var_bounds = ComponentMap()
        m.d2._disj_var_bounds = ComponentMap()
        m.d1.d._disj_var_bounds = ComponentMap()
        m.d2.d._disj_var_bounds = ComponentMap()
        m.d1._disj_var_bounds[m.x] = (None, None)
        m.d2._disj_var_bounds[m.x] = (3, 4)
        m.d1.d._disj_var_bounds[m.x] = (3, None)
        m.d2.d._disj_var_bounds[m.x] = (None, 5)
        TransformationFactory('contrib.enforce_disj_var_bounds').apply_to(m)
