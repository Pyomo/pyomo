...
class SemiBatchExperiment(Experiment):
    ...
    def label_model(self):
        ...
        m.experiment_outputs.update((m.Ca[t], self.data["Ca_meas"][f"{t}"]) for t in m.measT)
        m.experiment_outputs.update((m.Cb[t], self.data["Cb_meas"][f"{t}"]) for t in m.measT)
        m.experiment_outputs.update((m.Cc[t], self.data["Cc_meas"][f"{t}"]) for t in m.measT)
        m.experiment_outputs.update((m.Tr[t], self.data["Tr_meas"][f"{t}"]) for t in m.measT)
