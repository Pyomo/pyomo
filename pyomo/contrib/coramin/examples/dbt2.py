...
nlp = read_osil(
    'minlplib/osil/camshape800.osil', objective_prefix='obj_', constraint_prefix='con_'
)
relaxation = coramin.relaxations.relax(
    nlp,
    in_place=False,
    use_fbbt=True,
...
print(
    '{ub:<20}{lb:<20}{gap:<20}{avg_rng:<20}{time:<20}'.format(
        ub='UB', lb='LB', gap='% gap', avg_rng='Avg Var Range', time='Time'
    )
)
t0 = time.time()
print(
    '{ub:<20.3f}{lb:<20.3f}{gap:<20.3f}{avg_rng:<20.3e}{time:<20.3f}'.format(
        ub=ub, lb=lb, gap=gap, avg_rng=avg_bound_range, time=time.time() - t0
    )
)
