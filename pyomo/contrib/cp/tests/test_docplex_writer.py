...
class TestSolveModel(unittest.TestCase):
    ...
    def test_matching_problem(self):
        ...
        m.Observed = Param(m.Names, m.Names, m.Languages,
                           initialize={
                               (0, 1, 'English'): 1,
                               (1, 0, 'English'): 1,
                               (0, 2, 'English'): 1,
                               ...
                               (3, 4, 'English'): 1,
                               (4, 3, 'English'): 1,
                           }, default=0, mutable=True)# TODO: shouldn't need to
                                                      # be mutable, but waiting
                                                      # on #3045

        m.Expected = Param(m.People, m.People, m.Languages, initialize={
            ('P1', 'P2', 'English') :     1,
            ('P2', 'P1', 'English') :     1,
            ('P1', 'P3', 'English') :     1,
            ('P3', 'P1', 'English') :     1,
            ...
            ('P4', 'P5', 'English') :     1,
            ('P5', 'P4', 'English') :     1,
        }, default=0, mutable=True)# TODO: shouldn't need to be mutable, but
                                   # waiting on #3045

        m.person_name = Var(m.People, bounds=(0, max(m.Names)), domain=Integers)

        m.one_to_one = LogicalConstraint(expr=all_different(m.person_name[person] for
                                                            person in m.People))


        m.obj = Objective(expr=count_if(m.Observed[m.person_name[p1],
                                                   m.person_name[p2], l] ==
                                        m.Expected[p1, p2, l] for p1
                                        in m.People for p2 in
                                        m.People for l in
                                        m.Languages), sense=maximize)
        ...
        # names 5 and 6
        self.assertTrue(value(m.person_name['P6']) == 5 or
                         value(m.person_name['P6']) == 6)
        self.assertTrue(value(m.person_name['P7']) == 5 or
                        value(m.person_name['P7']) == 6)
