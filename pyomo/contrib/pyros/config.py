...
def resolve_keyword_arguments(prioritized_kwargs_dicts, func=None):
    """
    Resolve the keyword arguments to a callable in the event
    the arguments may have been passed in one or more possible
    ways.

    A warning-level message is logged (through the default PyROS
    logger) in the event an argument is specified in more than one
    way. In this case, the value provided through the means with
    the highest priority is selected.

    Parameters
    ----------
    prioritized_kwargs_dicts : dict
        Each entry maps a str to a dict of the keyword arguments
        passed via the means described by the str.
        Entries of `prioritized_kwargs_dicts` are taken to be
        provided in descending order of priority of the means
        by which the arguments may have been passed to the callable.
    func : callable or None, optional
        Callable to which the keyword arguments are/were passed.
        Currently, only the `__name__` attribute is used,
        for the purpose of logging warning-level messages.
        If `None` is passed, then the warning messages
