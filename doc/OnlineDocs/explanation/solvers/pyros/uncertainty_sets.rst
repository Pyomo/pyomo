...
.. list-table:: Mathematical definitions of PyROS uncertainty sets of dimension :math:`n`.
   ...
   * - :class:`~pyomo.contrib.pyros.uncertainty_sets.CardinalitySet`
     ...
     - :math:`\left\{ q \in \mathbb{R}^{n} \middle| \begin{array}{l} \exists\,\xi^+, \xi^- \in [0, 1]^n\,:\\ \quad \,q = q^{0} + \hat{q}^+ \circ \xi^+ - \hat{q}^- \circ \xi^- \\ \quad \displaystyle \sum_{i=1}^{n} (\xi_{i}^+ + \xi_{i}^-) \leq \Gamma \end{array} \right\}`
