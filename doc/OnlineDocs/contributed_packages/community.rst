...
creating a deep copy (``copy.deepcopy(community_map_object.community_map)``) (is highly recommended (or else any
destructive modifications could have unintended consequences).

Now, let's take a closer look at usage examples for the community map object generated by 'detect_communities':
...
``generate_structured_model`` Method
...
``visualize_model_graph`` Method
    ...
    .. doctest::
        ...
        >>> left_figure, pos = cmo.visualize_model_graph(type_of_graph='constraint')
        ...
        >>> right_figure, _ = cmo.visualize_model_graph(type_of_graph='bipartite', pos=pos)
        >>> plt.show()
