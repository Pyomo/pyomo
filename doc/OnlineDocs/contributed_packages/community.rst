...
.. doctest::
    ...
    Obviously, the above output is not user-friendly. We can use ``stringify_comunity_map`` if we want this
    same community map, but with the strings of the community members
    >>> print(stringify_community_map(model, type_of_community_map='bipartite'))
    {0: (['c1', 'c2'], ['x1', 'x2']),
     1: (['c3', 'c4', 'c5'], ['x3', 'x4'])}
...
.. doctest::
    ...
    Define some of the parameters to be used in the visualize_model_graph function call
    >>> comm_map = detect_communities(m, type_of_community_map='bipartite', random_seed=seed)

    Obtain a matplotlib figure (left_figure) and the graph layout (pos)
    >>> left_figure, pos = visualize_model_graph(model=m, community_map=comm_map, type_of_graph='bipartite')
    ...
    >>> seed = 5  # This will be used as the random seed for the heuristic Louvain community detection
