...
def solve_conic():
    ...
    m.k = pmo.block_tuple(
        [
            ...
            pmo.conic.primal_exponential.as_domain(r=None, x1=1, x2=m.u - m.t),
            pmo.conic.primal_exponential.as_domain(
                r=None, x1=1, x2=2 * m.v + m.w - m.t
            ),
            # exp(0.5u + log(0.1)) + exp(-v + log(2)) <= 1
            pmo.conic.primal_exponential.as_domain(
                r=None, x1=1, x2=0.5 * m.u + pmo.log(0.1)
            ),
            pmo.conic.primal_exponential.as_domain(r=None, x1=1, x2=-m.v + pmo.log(2)),
            # exp(-w) + exp(v-2u) <= 1
            pmo.conic.primal_exponential.as_domain(r=None, x1=1, x2=-m.w),
            pmo.conic.primal_exponential.as_domain(r=None, x1=1, x2=m.v - 2 * m.u),
        ]
    )
    ...
    print("objective: {0: .5f}".format(x + (y**2) * z))
    print("")
