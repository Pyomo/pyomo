...
def pyomo_maxVolCuboid(vertices):
    ...
    model.u = pmo.variable_list(pmo.variable_list(pmo.variable(lb=0.0) for j in range(m)) for i in range(2**n))
    ...
    for i in range(2**n):
        model.conv_hull.append(convex_hull_constraint(model, vertices, model.cuboid_vertices[i], i))
