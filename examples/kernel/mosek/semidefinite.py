...
def primal_sdo1():
    ...
    # CONSTRAINTS
    # Linear
    model.c1 = pmo.constraint( sum_product( barA1, model.X, index=list(range(n))) + model.x[0] == 1 )
    model.c2 = pmo.constraint( sum_product( barA2, model.X, index=list(range(n))) + model.x[1] + model.x[2] == 0.5 )
    # Conic
    ...
    model.psd_cone = pmo.conic.svec_psdcone.as_domain(x = [model.X[i]*scale[i] for i in range(n)])

    # OBJECTIVE
    model.obj = pmo.objective( sum_product(barC, model.X, index=list(range(n))) + model.x[0] )
...
def dual_sdo1():
    ...
    # CONSTRAINTS
    e1 = pmo.expression_list( pmo.expression(barC[i] - model.y[0]*barA1[i] - model.y[1]*barA2[i]) for i in range(n) )
    model.psd_cone = pmo.conic.svec_psdcone.as_domain(x = e1)

    e2 = pmo.expression_list( pmo.expression( c[i] - sum_product(a_T[i], model.y, index=[0, 1]) ) for i in range(3) )
