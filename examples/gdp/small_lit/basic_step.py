...
def build_gdp_model():
    ...
    model.disjuncts[1, 1].c = Constraint(
        expr=model.x1**2 + (1 / 4) * (model.x2 - 5) ** 2 <= 1
    )
    model.disjuncts[2, 1].c = Constraint(
        expr=model.x1**2 + (1 / 4) * (model.x2 - 2) ** 2 <= 1
    )
    model.disjuncts[3, 1].c = Constraint(
        expr=model.x1**2 + (1 / 4) * (model.x2 - 3.5) ** 2 <= 1
    )

    model.disjuncts[1, 2].c = Constraint(
        expr=(model.x1 - 5) ** 2 + (1 / 4) * (model.x2 - 2) ** 2 <= 1
    )
    model.disjuncts[2, 2].c = Constraint(
        expr=(model.x1 - 5) ** 2 + (1 / 4) * (model.x2 - 5) ** 2 <= 1
    )
    model.disjuncts[3, 2].c = Constraint(
