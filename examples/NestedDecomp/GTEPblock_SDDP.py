...
m.P_fuel = Param(m.i, m.t, N, default=0, initialize=P_fuel)
...
m.prob = Param(N, initialize=prob)
...
def node_filter(m,t,n,pn):
    return t in m.t and n in N_stage[t] and pn in PN[n]
m.n_pn_stage= Set(initialize=m.t*N*NP, filter=node_filter)

m.ngo_rn_par = Param(m.rn_r, m.n_pn_stage, default=0, initialize=0, mutable=True)
m.ngo_th_par = Param(m.th_r, m.n_pn_stage, default=0, initialize=0, mutable=True)
m.ngb_rn_par = Param(m.rn_r, m.n_pn_stage, default=0, initialize=0, mutable=True)
m.ngb_th_par = Param(m.th_r, m.n_pn_stage, default=0, initialize=0, mutable=True)
m.ngo_rn_par_k = Param(m.rn_r, m.n_pn_stage, m.iter, default=0, initialize=0, mutable=True)
m.ngo_th_par_k = Param(m.th_r, m.n_pn_stage, m.iter, default=0, initialize=0, mutable=True)
m.ngb_rn_par_k = Param(m.rn_r, m.n_pn_stage, m.iter, default=0, initialize=0, mutable=True)
m.ngb_th_par_k = Param(m.th_r, m.n_pn_stage, m.iter, default=0, initialize=0, mutable=True)
m.cost=Param(m.n_pn_stage,m.iter, default=0, initialize=0, mutable=True)
m.mltp_o_rn=Param(m.rn_r,m.n_pn_stage, m.iter, default=0, initialize=0, mutable=True)
m.mltp_o_th=Param(m.th_r,m.n_pn_stage,m.iter, default=0, initialize=0, mutable=True)
m.mltp_b_rn=Param(m.rn_r,m.n_pn_stage,m.iter, default=0, initialize=0, mutable=True)
m.mltp_b_th=Param(m.th_r,m.n_pn_stage,m.iter, default=0, initialize=0, mutable=True)
m.cost_t=Param(m.n_pn_stage,m.iter, default=0, initialize=0, mutable=True)

################ Block of Equations per time period ############################
def planning_block_rule(b,t,n,pn):
	...
	def obj_rule(_b):
		return m.if_[t]*\
			(sum(m.n_d[d]*m.hs* \
            	sum((m.VOC[i,t] + m.hr[i,r]*m.P_fuel[i,t,n] + m.EF_CO2[i]
			...
			+ sum(m.n_d[d]*m.hs*_b.su[th,r,d,s]*m.Qg_np[th,r] \
				*(m.f_start[th]*m.P_fuel[th,t,n] + m.f_start[th]*m.EF_CO2[th] \
...
m.Bl = Block(m.n_pn_stage, rule=planning_block_rule)
