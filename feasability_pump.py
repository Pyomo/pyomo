...
def add_L1_objective_function_constraints(model, setpoint_var_list):
...
def setup_milp_model(solve_data):
    ...
    del solve_data.milp_model.obj  # How do I do that more generally?
...
def solve_feasibility_pump(solve_data, config, write_back=True):
    ...
    while n_iter == 0 or \
        ...
        add_L1_objective_function_constraints(milp_model,
                                              nlp_model.var_list)
...
def solve_fixed_nlp(solve_data, config, write_back=True):
    ...
    opt_nlp = SolverFactory(config.nlp_solver)
    opt_nlp.solve(fixed_discretes_model, **config.nlp_solver_args)
    if write_back:
...
def create_linearized_objective_cuts(solve_data, config):
    ...
    """
    obj_vars = list(identify_variables(solve_data.working_model.obj))
    partial_derivatives = differentiate(solve_data.working_model.obj,
                                        wrt_list=obj_vars)
